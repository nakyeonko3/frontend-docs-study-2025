...
- 정적 렌더링
  ...
  - 매번 업데이트가 필요한 페이지(ex 데시보드)에는 정합하지 않음.
- 동적 렌더링
...
- **어떻게 해야 사용자가 원하는 데이터를 빠르게 전달할 수 있을까?(dynamic and static rendering)**
- **이전의 방식보다 얼마나 향상되었는지 측정할 수 있는 도구나 지표가 있을까?(sentry)**
...
- 어떻게 데이터를 가져오기 때문에 성능이 향상될까?
	- 성능 향상을 어떻게 측정할 수 있을까? sentry 이용해볼까? 로깅을 해보자
